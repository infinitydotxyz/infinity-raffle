...
contract ChainlinkV2VRF is Ownable, VRFConsumerBaseV2, RNG {
  ...
  function requestRandomNumbers(uint32 numRandomNumbers) external override onlyOwner returns (uint256) {
    require(numRandomNumbers > 0, 'Must request at least one random number');
    require(numRandomNumbers <= MAX_RANDOM_NUMBERS, 'Exceeded max number of random numbers');
  ...
  function isRequestFulfilled(uint256 requestId) external view override returns (bool isFulfilled) {
    return randomNumbers[requestId].length > 0;
  }

  function getFulfilledRequest(uint256 requestId) external view override returns (uint256[] memory randomWords) {
